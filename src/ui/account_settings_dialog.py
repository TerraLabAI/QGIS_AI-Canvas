...
class AccountSettingsDialog(QDialog):
    ...
    def _build_subscription_card(self, sub: dict) -> QFrame:
        ...
        grid.addWidget(self._field_label(tr("Usage")), row, 0)
        usage_label = QLabel(f"{used} / {limit}")
        usage_label.setStyleSheet("font-size: 12px; color: palette(text);")
        grid.addWidget(usage_label, row, 1)
        row += 1
        ...
        # Color: green when under 80%, orange 80-99%, red at 100%
        ...
        remaining_label = QLabel(
            f"{remaining} {tr('remaining')}" if remaining > 0
            else tr("Limit reached")
        )
        remaining_label.setStyleSheet(
            f"font-size: 10px; color: {'palette(text)' if remaining > 0 else BRAND_RED};"
        )
        card_layout.addWidget(remaining_label)

        # Reset date or free plan notice
        period_end = sub.get("current_period_end", "")
        is_free = plan == "free" and status != "trialing"
        if is_free:
            reset_label = QLabel(tr("No reset on free plan"))
            reset_label.setStyleSheet("font-size: 10px; color: rgba(255,255,255,0.5);")
        elif period_end:
